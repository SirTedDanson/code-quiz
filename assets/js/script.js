...
// ask a quiz question, clear quiz
function askQuestion () {
  clearQuiz ();
  nextQuestion (randomQuestions[currentQuestion]);
};

// -------------------------- GENERATE QUIESTIONS AND ANSWERS-----------------------
function nextQuestion (question) {
  ...
  question.answers.forEach(answer => {
    ...
    // set dataset to correct if answer was correct
    if(answer.correct) {
...
var answerFunction = function (event) {
  ...
  // if answer correct add score
  if (userAnswer == correctAnswer) {
      currentScore += 10;
      answerAlert.innerHTML = "<h3>Correct!</h3>";
      qBody.appendChild(answerAlert);
  // if answer incorrect subtract score and time     
  } else {
      currentScore -= 5;
      timeLeft -= 5;
      answerAlert.innerHTML = "<h3>Wrong!</h3>";
      qBody.appendChild(answerAlert);
  };
...
function captureScore () {
  ...
  timeLeft = -1;
  qTimer.textContent = "";
  finalScore = Math.ceil(Math.max(0, (currentScore + scoreBonus)));
  submitScore();
};

// ------------------------ LOAD SUBMIT INITIALS SCREEN---------------------
...
// prompt user if no initials where entered --------------------
function emptyField () {
...
var saveScore = function() {
  ...
  var finalScore = Math.ceil(Math.max(0, (currentScore + scoreBonus)));
  var userRoundScore = {
    name: userInitials,
    score: finalScore
  };
...
var highScoreScreen = function () {
  ...
  // disable View High Score Link when on highscore page
  ...
  // clear high scores when "clear high scores" button is pressed
...
// load the highscores from storage when page loads
loadHighScores ();
...
const questions = [
  ...
  },
  {
    ...
    answers: [
      ...
      { text: '4. Keyword Wich Refers To The Object It Belongs To', correct: true}
    ]
  },
  {
